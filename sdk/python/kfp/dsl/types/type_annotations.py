...
def get_input_or_output_marker(
        typ) -> Union[InputAnnotation, OutputAnnotation, None]:
    if not is_Input_Output_artifact_annotation(typ):
        return None
...
def strip_optional_if_present(annotation: T) -> T:
...
def get_inner_type(annotation) -> None:
    """
    Return the inner type of a generic annotation.
    For Union or Optional types with multiple inner types, a tuple of types is returned.
    """
