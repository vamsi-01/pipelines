...
def func_to_root_annotation_symbols_for_artifacts(
    ...
    """Gets a map of parameter name to the root symbol used to reference an annotation for a function. For example, the annotation `type_module.Artifact` has the root symbol `type_module` and the annotation `Artifact` has the root symbol `Artifact`."""
    module_node = ast.parse(_get_function_source_definition(func))
...
def _get_command_and_args_for_lightweight_component(
    ...
    imports_source = [
        ...
        'from kfp.dsl import *',
        'from typing import *',
    ] + get_custom_artifact_type_import_statements(func)
