...
class InputListOfArtifactsPlaceholder(Placeholder):
    ...
    def __getattribute__(self, __name: str) -> Any:
        if __name in {'name', 'uri', 'metadata', 'path'}:
            raise AttributeError(
                f'Cannot access an attribute on a list of artifacts in a Custom Container Component. Found refence to attribute `{__name}` on `{self.input_name}`. Please pass the whole list of artifacts only.'
            )
        else:
    ...
    def __getitem__(self, k: int) -> None:
        raise KeyError(
            f'Cannot access individual artifacts in a list of artifacts. Found access to element {k} on `{self.input_name}`. Please pass the whole list of artifacts only.'
        )
...
# TODO: output lists of placeholders -- add to PRIMITIVE_OUTPUT_PLACEHOLDERS
