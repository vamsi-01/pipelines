...
class TestLoadBaseModelHelper(parameterized.TestCase):
    ...
    def test_load_primitive_other_iterables(self):
        self.assertEqual(
            self.no_alias_load_base_model_helper(abc.Sequence[int], [1]), [1])
        self.assertEqual(
            self.no_alias_load_base_model_helper(abc.MutableSequence[float],
                                                 [1.0]), [1.0])
        self.assertEqual(
    ...
    def test_load_mapping(self):
        self.assertEqual(
            self.no_alias_load_base_model_helper(abc.Mapping[str, str],
                                                 {'a': 'a'}), {'a': 'a'})
        self.assertEqual(
            self.no_alias_load_base_model_helper(abc.MutableMapping[str, int],
                                                 {'a': 1}), {'a': 1})
        self.assertEqual(
