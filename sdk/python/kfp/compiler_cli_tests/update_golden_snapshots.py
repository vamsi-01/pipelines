...
def update_snapshots(pipelines: bool, components: bool):
    if pipelines:
        ...
        for file in python_files:
            print(f'Updating golden snapshots for pipeline in {file}.')
            python_file = os.path.join(TEST_DATA_DIR, file)
            call_and_raise(f'python {python_file}')
    if components:
        for data in cli_test.SUPPORTED_COMPONENTS_COMPILE_TEST_CASES:
            file = data['file']
            component_name = data['component_name']
            print(
                f'Updating golden snapshots for component "{component_name}" in {file}.'
            )
