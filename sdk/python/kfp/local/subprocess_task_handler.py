...
class SubprocessTaskHandler(task_handler_interface.TaskHandlerInterface):
    ...
    def validate_image(self, image: str) -> None:
        if 'python' not in image:
            warnings.warn(
                f"You may be attemping to run a in a Python environment a task that uses custom or non-Python base image '{image}'. This is discouraged and may have incorrect behavior or missing dependencies.",
                RuntimeWarning,
            )
    ...
    def validate_not_container_component(
        ...
        if component_factory.EXECUTOR_MODULE not in ' '.join(full_command):
            raise RuntimeError(
...
def environment(use_venv: bool) -> str:
    """Context manager that handles the virtual environment used for the
    subprocess.
