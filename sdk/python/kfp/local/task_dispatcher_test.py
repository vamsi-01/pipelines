...
class TestInvalidArguments(testing_utilities.LocalRunnerEnvironmentTestCase):
    ...
    def test_extra_argument(self, runner):
        ...
        with self.assertRaisesRegex(
                ValueError,
                r"Component 'comp-identity' got an unexpected input: 'y'\."):
...
# class TestRunLoadedComponent(testing_utilities.LocalRunnerEnvironmentTestCase):

#     def test(self):
#         local.init(runner=local.SubprocessRunner(use_venv=False), cleanup=False)
...
# class TestCleanupBehavior(testing_utilities.LocalRunnerEnvironmentTestCase):

#     def test_output_cleanup_root_starts_empty(self):
...
#             assert 'my_pipeline_root' not in dataset_out.path
...
#         task = multiple_io(string_in='my string')

#         # outputs are still attached
#         self.assertEqual(task.outputs['Output'], 'my string')
#         # self.assertIsInstance(task.outputs['int_out'], 1)
#         self.assertIsInstance(task.outputs['dataset_out'], Dataset)

#         # outputs but the output directory should not exist
#         self.assertFalse(os.path.exists(pipeline_root))

#     def test_output_cleanup_with_existing_files(self):
#         pipeline_root = os.path.join(
#             os.path.abspath(os.path.dirname(__file__)),
#             'my_pipeline_root',
#         )
#         existing_file_path = os.path.join(pipeline_root, 'existingfile.txt')
#         os.makedirs(os.path.dirname(existing_file_path))
#         with open(existing_file_path, 'w+') as f:
#             f.write('I should not be cleaned up!')

#         def remove_pipeline_root():
#             os.remove(existing_file_path)
#             os.rmdir(pipeline_root)

#         self.addCleanup(remove_pipeline_root)
...
#         task = identity(x='my string')

#         # outputs are still attached
#         self.assertEqual(task.output, 'my string')

#         # and the output directory should contain only the files that existed before
#         self.assertListEqual(os.listdir(pipeline_root), ['existingfile.txt'])
...
#         with self.assertRaisesRegex(
#                 dsl.types.type_utils.InconsistentTypeException,
#                 "Incompatible argument passed to the input 'x' of component 'comp-identity': Argument type 'NUMBER_INTEGER' is incompatible with the input type 'STRING'"
#         ):
#             identity(x=1)
