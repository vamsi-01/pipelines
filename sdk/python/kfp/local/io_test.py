...
class IOStoreTest(unittest.TestCase):
    ...
    def test_get_put_task_output(self):
        store = io.IOStore()
        ...
        # test doesn't remove by getting twice
        self.assertEqual(
            store.get_task_output('my-task', 'baz'),
            'bat',
        )

    def test_get_put_parent_input(self):
        store = io.IOStore()
    ...
    def test_put_get_task_output_with_artifact(self):
