...
def run_importer(
    ...
    artifact_cls = get_artifact_class_for_schema_title(
...
def get_importer_uri(
    ...
    if uri.startswith(artifact_types.GCS_REMOTE_PREFIX) or uri.startswith(
        ...
        warnings.warn(
            f"It looks like you're using the remote file '{uri}' in a 'dsl.importer'. Note that you will only be able to read and write to/from local files using 'artifact.path' when running a pipeline locally."
        )

    return uri


def get_artifact_class_for_schema_title(schema_title: str) -> dsl.Artifact:
