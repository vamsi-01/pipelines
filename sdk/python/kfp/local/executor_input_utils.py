...
def get_local_pipeline_resource_name(pipeline_name: str) -> str:
    """The pipeline name provided by PipelineSpec.pipelineInfo.name."""
    timestamp = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S-%f')
    return f'{pipeline_name}-{timestamp}'


def get_local_task_resource_name(component_name: str) -> str:
    """The component name provided as the key for the component's ComponentSpec
    message.
...
def construct_local_task_root(
    ...
    """Constructs the task root directory for a task."""
    return os.path.join(
        pipeline_root,
