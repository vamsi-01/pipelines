...
def dataproc_analyze_op(project, region, cluster_name, schema, train_data,
                        output):
    ...
    return dataproc_submit_pyspark_op(
        ...
        args=[
            '--output',
            str(output), '--train',
            str(train_data), '--schema',
            str(schema)
        ])


def dataproc_transform_op(project, region, cluster_name, train_data, eval_data,
    ...
    return dataproc_submit_pyspark_op(
        ...
        region=region,
        cluster_name=cluster_name,
        main_python_file_uri=os.path.join(_PYSRC_PREFIX, 'transform_run.py'),
        args=[
            '--output',
            str(output), '--analysis',
            str(analysis), '--target',
...
def dataproc_predict_op(project, region, cluster_name, data, model, target,
                        analysis, output):
...
def xgb_train_pipeline(
    ...
    with dsl.ExitHandler(
            exit_op=dataproc_delete_cluster_op(
