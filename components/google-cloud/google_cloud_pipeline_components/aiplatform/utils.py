...
RESOURCE_TO_METADATA_TYPE = {
    aiplatform.datasets.dataset._Dataset:
        'google.VertexDataset' + '@' + DEFAULT_SCHEMA_VERSION,  # pylint: disable=protected-access
    aiplatform.Model:
        'google.VertexModel' + '@' + DEFAULT_SCHEMA_VERSION,
    aiplatform.Endpoint:
        'google.VertexEndpoint' + '@' + DEFAULT_SCHEMA_VERSION,
    aiplatform.BatchPredictionJob:
        'google.VertexBatchPredictionJob' + '@' + DEFAULT_SCHEMA_VERSION
}
...
def get_proto_plus_deserializer(
        annotation: Any) -> Optional[Callable[..., str]]:
...
def map_resource_to_metadata_type(
    ...
    if is_serializable_to_json(mb_sdk_type):
        return 'exported_dataset', 'google.VertexDataset' + '@' + DEFAULT_SCHEMA_VERSION

    # handles the case of imported datasets
    if mb_sdk_type == '_Dataset':
        return 'dataset', 'google.VertexDataset' + '@' + DEFAULT_SCHEMA_VERSION
...
def convert_method_to_component(cls: aiplatform.base.VertexAiResourceNoun,
    ...
    if output_type:
        ...
        output_specs[output_metadata_name] = output_spec
    ...
    def component_yaml_generator(**kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if serializer:
                param_type = str
                if not isinstance(
                        value, kfp.components.pipeline_channel.PipelineChannel):
            ...
            if isinstance(value, kfp.components.pipeline_channel.PipelineChannel
                         ) or serializer:
                ...
                input_specs[key] = structures.InputSpec(
                ...
                if is_mb_sdk_resource_noun_type(param_type):
                    ...
                    input_args.append(
                        placeholders.InputValuePlaceholder(input_name=key))
        ...
        with open(component_path, 'w') as f:
            yaml.safe_dump(
                json_format.MessageToDict(component_spec.to_pipeline_spec()), f)
