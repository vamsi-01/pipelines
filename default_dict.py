...
class InputDict(Generic[A]):
    ...


class OutputDict(Generic[A]):
    ...
...
@dsl.component
def train(datasets: InputDict[Dataset], models: OutputDict[Model]):
...
@dsl.component
def parallelfor_collector(datasets: InputDict[Dataset]):
...
# list v dict tradeoff:
## dict advantage:
## - int and string keys
## - no requirement for sequential access

## list advantage
## - no ExecutorInput IR change

# data structures
# parallelfor input access
## cannot use key... technical challenge but also unclear what users would want
## shouldn't put on artifact class
